...
from basic_blocks.basic_block import RBF
...
class conj_edge_RBF(nn.Layer):
    """
    Conj Float Encoder using Radial Basis Functions
    """

    def __init__(self, bond_float_names, embed_dim, rbf_params=None):
        super(conj_edge_RBF, self).__init__()
        self.bond_float_names = bond_float_names

        if rbf_params is None:
            self.rbf_params = {
                'all_arom': (np.arange(0, 1, 0.1), 10.0),  # (centers, gamma)
                'hemi_arom': (np.arange(0, 1, 0.1), 10.0),  # (centers, gamma)
                'arom_conj': (np.arange(0, 1, 0.1), 10.0),  # (centers, gamma)
                'no_arom': (np.arange(0, 1, 0.1), 10.0),  # (centers, gamma)
            }
        else:
            self.rbf_params = rbf_params
