...
def main(args):
    ...
    modedir = f"{args.model_dir_spa}/{args.init_model}"
    compound_encoder.set_state_dict(paddle.load(modedir))
    print('Load state_dict from %s' % modedir)

    list_test_loss = []
    ###
    def spatial_block_freezing(model, epoch, total_epochs):
        model.freeze_bond_angle_only()
        phase = "energe task"
