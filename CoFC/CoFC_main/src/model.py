...
from basic_blocks.basic_block import MLP


class encoder_with_head(nn.Layer):
    """
    ...
    def __init__(self, model_config, compound_encoder):
        super(encoder_with_head, self).__init__()
    ...
    def forward(self, atom_bond_graphs, bond_angle_graphs, dihedral_angle_graphs, conj_graph):
        """
        Define the output head.
        Returns:
            pred: the model prediction.
        """
        node_repr, edge_repr, plane_repr, graph_repr = self.compound_encoder(atom_bond_graphs, bond_angle_graphs, dihedral_angle_graphs, conj_graph)
